...
class Theory:
    ...
    def integrate_siegert(self):
        ...
        dt = self.params['dt']
        T = self.params['T']
        rate_ext = self.network.rates
        rate_ext = rate_ext.sort_index()

        K = copy(self.network.K).T
        J = copy(self.network.J).T
        K_areas = K.loc[self.network.area_list][self.network.area_list]
        J_areas = J.loc[self.network.area_list][self.network.area_list]
        ...
        drive = {}
        for pop, r in rate_ext.iteritems():
            drive[pop] = nest.Create(
                'siegert_neuron', 1, params={'rate': r, 'mean': r}
                )

        # create neurons representing populations
        neurons = {}
        for pop, r in rate_ext.iteritems():
            neurons[pop] = nest.Create('siegert_neuron', 1)
        # neurons = nest.Create('siegert_neuron', dim)
        # external drive
        K_ext = K['external', 'external', 'external', 'external']
        J_ext = J['external', 'external', 'external', 'external']
        drift_factor = tau * K_ext * J_ext
        diffusion_factor = tau * K_ext * J_ext**2

        for i, (pop, r) in enumerate(rate_ext.iteritems()):
            syn_dict = {'drift_factor': drift_factor.loc[pop],
                        'diffusion_factor': diffusion_factor.loc[pop],
                        'receptor_type': 0}
            if self.network.params['USING_NEST_3']:
                syn_dict['synapse_model'] = 'diffusion_connection'
            else:
                syn_dict['model'] = 'diffusion_connection'
            nest.Connect(drive[pop], neurons[pop], 'all_to_all', syn_dict)

        # # external DC drive (expressed in mV)
        # DC_drive = {}
        # for i, (pop, r) in enumerate(rate_ext.iteritems()):
        #     DC_drive[pop] = nest.Create(
        #         'siegert_neuron', 1, params={'rate': 1., 'mean': 1.})

        # C_m = self.network.params['neuron_params']['single_neuron_dict']['C_m']
        # dc_drive_drift = 1e3 * tau / C_m * np.array(np.array([self.network.add_DC_drive.values]).transpose()).reshape(dim, 1)
        # syn_dict = {'drift_factor': 0,  # dc_drive_drift
        #             'diffusion_factor': 0.,
        #             'receptor_type': 0}
        # if self.network.params['USING_NEST_3']:
        #     syn_dict['synapse_model'] = 'diffusion_connection'
        ...
        #     syn_dict['model'] = 'diffusion_connection'
        # for i, (pop, r) in enumerate(rate_ext.iteritems()):
        #     nest.Connect(DC_drive[pop], neurons[pop], 'all_to_all', syn_dict)

        drift_factor = tau * K_areas * J_areas
        diffusion_factor = tau * K_areas * J_areas**2
        for source_pop, _ in rate_ext.iteritems():
            for target_pop, _ in rate_ext.iteritems():
                if self.network.params['USING_NEST_3']:
                    nest.Connect(
                            neurons[source_pop],
                            neurons[target_pop],
                            'all_to_all',
                            syn_spec={'drift_factor': drift_factor.loc[target_pop][source_pop],
                                      'diffusion_factor': diffusion_factor.loc[target_pop][source_pop],
                                      'receptor_type': 0,
                    ...
                    nest.Connect(
                            ...
                            neurons[pop],
                            'all_to_all',
                            {'drift_factor': drift_factor.loc[target_pop][source_pop],
                             'diffusion_factor': diffusion_factor.loc[target_pop][source_pop],
                             'receptor_type': 0,
        ...
        # # network connections
        # syn_dict = {'drift_factor': (tau * K_areas * J_areas).values,
        #             'diffusion_factor': (tau * K_areas * J_areas**2).values,
        #             'receptor_type': 0}
        # if self.network.params['USING_NEST_3']:
        #     syn_dict['synapse_model'] = 'diffusion_connection'
        # else:
        ...
        for nsim in range(num_iter):
            ...
            for i, (pop, r) in enumerate(rate_ext.iteritems()):
                if self.network.params['USING_NEST_3']:
                    nest.SetStatus(neurons[pop], {'rate': initial_rates[i]})
                else:
                    nest.SetStatus([neurons[pop]], {'rate': initial_rates[i]})
            # simulate
            nest.Simulate(T + dt)
            ...
            if self.network.params['USING_NEST_3']:
                res = np.array([np.insert(data['rate'][ind][np.where(data['senders'][ind] == n.global_id)],
                                          0,
                                          initial_rates[i])
                                for i, (pop, n) in enumerate(neurons.items())])
            else:
        ...
        if num_iter == 1:
            return list(K_areas.index), rates[0]
        else:
            return list(K_areas.index), rates
