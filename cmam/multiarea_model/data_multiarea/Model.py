...
def compute_Model_params(CLUSTER, out_label='', mode='default'): 
    """ 
    ...
    if mode == 'default':
        ...
        net_params = nested_update(net_params, custom_params)
    ...
    stim_params = net_params['stim_params']
    ...
    # FB threshold
    fb_threshold = conn_params['fb_threshold']
    # FF threshold
    ff_threshold = conn_params['ff_threshold']
    ...
    # synaptic weight (mV) for stimulation input
    PSP_stim = stim_params['PSP_stim']
    # synaptic weight (mV) for stimulation input
    PSP_cluster = stim_params['PSP_cluster']
    ...
    # relative SD of normally distributed synaptic weights
    PSC_stim_rel_sd_normal = stim_params['PSP_sd']
    ...
    def num_CC_synapses(target_area, target_pop, source_area, source_pop):
        ...
        if (source_area in Coco_Data[target_area] and
            ...
            if Coco_Data[target_area][source_area]['target_pattern'] is not None:
                ...
                # yes
                if -1 in tp:
                    if (SLN_Data[target_area][source_area] > fb_threshold and
                            SLN_Data[target_area][source_area] <= ff_threshold):
                        T_hierarchy = termination_layers2['C']
                    elif SLN_Data[target_area][source_area] < fb_threshold:
                        T_hierarchy = termination_layers2['M']
                    elif SLN_Data[target_area][source_area] > ff_threshold:
        ...
        for target_area in area_list:
            for source_area in area_list:
                if (target_area != source_area and source_area in Coco_Data[target_area] and
                        SLN_Data[target_area][source_area] < fb_threshold):
                    syn_I = 0.0
                    syn_E = 0.0
    ...
    for area in area_list:
        ...
        vector[-1] = ext_syn
        # solution is the indegree
        solution, residues, rank, s = np.linalg.lstsq(
            nonvisual_fraction_matrix, vector)

        indegree_pulvinar = solution[0] * CLUSTER[area]['pulvinar_frac_type_4_vs_3']
        pulvinar_target_layers = CLUSTER[area]['pulvinar_target_layers']
        if area != 'TH':
            for target_pop in ['23E', '23I', '4E', '4I', '5E', '5I', '6E', '6I']:
                synapse_numbers[area][target_pop]['pulvinar'] = {}
                synapse_numbers[area][target_pop]['pulvinar']['pulvinar'] = 0.
        else:
            for target_pop in ['23E', '23I', '5E', '5I', '6E', '6I']:
                synapse_numbers[area][target_pop]['pulvinar'] = {}
                synapse_numbers[area][target_pop]['pulvinar']['pulvinar'] = 0.
        for target_syn_layer, target_population in synapse_to_cell_body_basis.items():
            if target_syn_layer in pulvinar_target_layers:
                for target_pop, prob in target_population.items():
                    if (target_pop == '4E' or target_pop == '4I') and area == 'TH':
                        continue
                    synapse_numbers[area][target_pop]['pulvinar']['pulvinar'] += prob * indegree_pulvinar * neuronal_numbers[area][target_pop]

        for i, pop in enumerate(structure[area]):
            synapse_numbers[area][pop]['external'] = {
                'external': solution[i] * neuronal_numbers[area][pop]}
            # 3.3 % of al connections are from non-visual and
            # subcortical structures, 28,5 % of all connections are
            # from within a patch. The number of external
            # subcortical/non-visual connections should be roughly 3.3 /
            # (28.5 + 3.3) * K_ext
            synapse_numbers[area][pop]['stim'] = {
                'stim': stim_params['frac_type_4_vs_3'] *
                solution[i] * neuronal_numbers[area][pop]
                }
            if pop in CLUSTER[area]['cluster_stim_target_population']:
                synapse_numbers[area][pop]['cluster_stim'] = {
                    'cluster_stim': stim_params['cluster_frac_type_4_vs_3'] *
                    solution[i] * neuronal_numbers[area][pop]
                    }
            else:
                synapse_numbers[area][pop]['cluster_stim'] = {
    ...
    synapse_numbers['TH']['4E']['stim'] = {'stim': 0.0}
    synapse_numbers['TH']['4I']['stim'] = {'stim': 0.0}

    synapse_numbers['TH']['4E']['cluster_stim'] = {'cluster_stim': 0.0}
    synapse_numbers['TH']['4I']['cluster_stim'] = {'cluster_stim': 0.0}
            ...
            if target_pop in ['5E']:
                synapse_numbers[target_area][target_pop]['external'][
                    'external'] = fac_nu_ext_5E * synapse_numbers[target_area][target_pop][
                        'external']['external']
                synapse_numbers[target_area][target_pop]['stim'][
                    'stim'] = fac_nu_ext_5E * synapse_numbers[target_area][target_pop][
                        'stim']['stim']
                synapse_numbers[target_area][target_pop]['cluster_stim'][
                    'cluster_stim'] = fac_nu_ext_5E * synapse_numbers[target_area][target_pop][
                        'cluster_stim']['cluster_stim']
            if target_pop in ['6E']:
                synapse_numbers[target_area][target_pop]['external'][
                    'external'] = fac_nu_ext_6E * synapse_numbers[target_area][target_pop][
                        'external']['external']
                synapse_numbers[target_area][target_pop]['stim'][
                    'stim'] = fac_nu_ext_6E * synapse_numbers[target_area][target_pop][
                        'stim']['stim']
                synapse_numbers[target_area][target_pop]['cluster_stim'][
                    'cluster_stim'] = fac_nu_ext_6E * synapse_numbers[target_area][target_pop][
                        'cluster_stim']['cluster_stim']

    synapse_numbers['TH']['23E']['external']['external'] *= fac_nu_ext_TH
    synapse_numbers['TH']['5E']['external']['external'] *= fac_nu_ext_TH

    synapse_numbers['TH']['23E']['stim']['stim'] *= fac_nu_ext_TH
    synapse_numbers['TH']['5E']['stim']['stim'] *= fac_nu_ext_TH

    synapse_numbers['TH']['23E']['cluster_stim']['cluster_stim'] *= fac_nu_ext_TH
    synapse_numbers['TH']['5E']['cluster_stim']['cluster_stim'] *= fac_nu_ext_TH
    ...
    for target_area in area_list:
        for target_pop in population_list:
            ...
            synapse_weights_mean[target_area][target_pop]['stim'] = {
                'stim': PSC_e_over_PSP_e * PSP_stim}
            synapse_weights_sd[target_area][target_pop]['stim'] = {
                'stim': PSC_stim_rel_sd_normal * PSC_e_over_PSP_e * PSP_stim
                }
            synapse_weights_mean[target_area][target_pop]['cluster_stim'] = {
                'cluster_stim': PSC_e_over_PSP_e * PSP_cluster}
            synapse_weights_sd[target_area][target_pop]['cluster_stim'] = {
                'cluster_stim': PSC_stim_rel_sd_normal * PSC_e_over_PSP_e * PSP_cluster
                }
            synapse_weights_mean[target_area][target_pop]['pulvinar'] = {
                'pulvinar': PSC_e_over_PSP_e * PSP_cluster}
            synapse_weights_sd[target_area][target_pop]['pulvinar'] = {
    ...
    def clusterize_2(d, cluster_params, remove_TH_layer_4=False): # LVD 
        ...
        for a_t, d1 in d.items():
            ...
            num_cluster = cluster_params[a_t]['Q']
            for pop_t, d2 in d1.items():
                if pop_t in population_list:
                    for c0 in cluster:
                        new_dict[a_t][pop_t[:-1]][pop_t[-1]][c0] = d2 / num_cluster
        return new_dict

    def clusterize_4(d, cluster_params, syn=False):
        new_dict = nested_dict()
        for a_t, d1 in d.items():
                ...
                for c0 in cluster_t:
                    for a_s, d3 in d2.items():
                        ...
                        for pop_s, d4 in d3.items():
                            val = d4 * divider
                            if pop_s in population_list:
                                for c1 in cluster_s:
                                    new_dict[a_t][pop_t[:-1]][pop_t[-1]][c0][a_s][pop_s[:-1]][pop_s[-1]][c1] = val
                            else:
    ...
    def split_layer_type(d, remove_TH_layer_4=False):
        ...
        for a_t, d1 in d.items():
            for pop_t, d2 in d1.items():
                if remove_TH_layer_4 and a_t == 'TH' and pop_t[:-1] == '4':
                    continue
                for a_s, d3 in d2.items():
                    for pop_s, d4 in d3.items():
                        if remove_TH_layer_4 and a_s == 'TH' and pop_s[:-1] == '4':
                            continue
                        if pop_s in population_list:
                            new_dict[a_t][pop_t[:-1]][pop_t[-1]][a_s][pop_s[:-1]][pop_s[-1]] = d4
                        else:
                            new_dict[a_t][pop_t[:-1]][pop_t[-1]][a_s][pop_s][pop_s] = d4
        return new_dict

    def split_layer_type_2(d, remove_TH_layer_4=False):
        new_dict = nested_dict()
        for a, d1 in d.items():
            for pop, d2 in d1.items():
                if remove_TH_layer_4 and a == 'TH' and pop[:-1] == '4':
                    continue
                if pop in population_list:
                    new_dict[a][pop[:-1]][pop[-1]] = d2
        return new_dict
    ...
    with open(os.path.join(basepath,
                           '.'.join(('_'.join((prefix,
                                               ...
                                               out_label)),
                                     'json'))), 'w') as f:
        json.dump(collected_data['structure'], f)
    ...
    with open(os.path.join(basepath,
                           '.'.join(('_'.join((prefix,
                                               ...
                                               out_label)),
                                     'json'))), 'w') as f:
        json.dump(collected_data['distances'], f)
    ...
    with open(os.path.join(basepath,
                           '.'.join(('_'.join((prefix,
                                               ...
                                               out_label)),
                                     'json'))), 'w') as f:
        json.dump(collected_data['area_list'], f)
