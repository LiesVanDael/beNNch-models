...
def calculate_K_synapses_nested_dict(  # for N with 3 levels (no clusters)
        synapses,
        area_list,
        structure,
        K_stable_path,
    ...
    # Collect all source areas
    for a_t, d1 in synapses.items():
        for layer_t, d2 in d1.items():
            for pop_t, d3 in d2.items():
                for a_s in d3:
                    source_areas.add(a_s)

    external_input = list(source_areas - set(area_list))
    i = 0
    
    # target area
    for a1 in area_list:
        # target layer and target population
        for s1 in structure[a1]:
            layer1 = s1[:-1]
            pop1 = s1[-1]
            j = 0
            # source area
    ...
    for a_t, d1 in x.items():
        for layer_t, d2 in d1.items():
            for pop_t, d3 in d2.items():
                N_value = N[a_t][layer_t][pop_t]
                for a_s, d4 in d3.items():
                    for layer_s, d5 in d4.items():
                        for pop_s, x_value in d5.items():
                            synapses[a_t][layer_t][pop_t][a_s][layer_s][pop_s] = x_value * N_value

    return x, synapses


def calculate_K_synapses_nested_dict_N4( # for N with 4 levels (including clusters)
        synapses,
        area_list,
        CLUSTER,
        structure,
                ...
                for a_s, d4 in d3.items():
                    ...
                    for layer_s, d5 in d4.items():
                        for pop_s, n_synapses in d5.items():
                            ...
                            n_s = N.get(a_s, {}).get(layer_s, {}).get(pop_s, 0)
                            indeg = n_synapses / n_t if n_t > 0 else 0.0
                            indegrees[a_t][layer_t][pop_t][a_s][layer_s][pop_s] = indeg
                            indegrees_areas[a_t][a_s] += indeg

                            outdeg = n_synapses / n_s if n_s > 0 else 0.0
                            outdegrees[a_t][layer_t][pop_t][a_s][layer_s][pop_s] = outdeg
                            outdegrees_areas[a_t][a_s] += outdeg

    return (
        indegrees.to_dict(),
        indegrees_areas.to_dict(),
        outdegrees.to_dict(),
...
def filter_nested_dict_3(d, remove_TH_layer_4=False, remove_total=False):
    ...
    out = {}
    for a, d1 in d.items():  # area
        if remove_TH_layer_4 and a == 'TH' and '4' in d1:
            d1 = {l: pop for l, pop in d1.items() if l != '4'}
        if remove_total:
            d1 = {l: pop for l, pop in d1.items() if l != 'total'}
        out[a] = d1
    return out


def filter_nested_dict_6(d, remove_TH_layer_4=False, remove_total=False):
    """Filter both target and source sides in nested dicts."""
    out = {}
    for t_area, t_layers in d.items():
        if remove_TH_layer_4 and t_area == 'TH' and '4' in t_layers:
            t_layers = {l: pop for l, pop in t_layers.items() if l != '4'}
        if remove_total:
            t_layers = {l: pop for l, pop in t_layers.items() if l != 'total'}

        new_t_layers = {}
        for t_layer, t_pops in t_layers.items():
            new_t_pops = {}
            for t_pop, s_areas in t_pops.items():
                new_s_areas = {}
                for s_area, s_layers in s_areas.items():
                    if remove_TH_layer_4 and s_area == 'TH' and '4' in s_layers:
                        s_layers = {k: v for k, v in s_layers.items() if k != '4'}
                    if remove_total:
                        s_layers = {k: v for k, v in s_layers.items() if k != 'total'}
                    new_s_areas[s_area] = s_layers
                new_t_pops[t_pop] = new_s_areas
            new_t_layers[t_layer] = new_t_pops
        out[t_area] = new_t_layers
    return out
...
def load_degree_data(fn):
    ...
    syn_stim = dat['synapse_numbers_stim']
    indegrees = nested_dict()
    indegrees_stim = nested_dict()
    outdegrees = nested_dict()
    ...
    for target_area, target_pop, ext_pop in product(complete_area_list,
                                                    population_list, ['stim']):
        numT = num[target_area][target_pop]
        if numT > 0.0:
            indegrees_stim[target_area][target_pop]['stim'][ext_pop] = syn_stim[
                target_area][target_pop]['stim'][ext_pop] / numT
        else:
            indegrees_stim[target_area][target_pop]['stim'][ext_pop] = 0.0
    ...
    return (indegrees.to_dict(), indegrees_areas,
            outdegrees.to_dict(), outdegrees_areas, indegrees_stim.to_dict())
...
def matrix_to_dict(m, area_list, structure, external=None, stimulus=None):
    """
    ...
    external: numpy.ndarray or dict
        ...
        Defaults to None.
    """
    # Cannot dealt with stimulus and external input at the same time
    assert (external is None) or (stimulus is None)
    dic = nested_dict()
    for area, area2 in product(area_list, area_list):
        mask = create_mask(
            structure, target_areas=[area], source_areas=[area2], external=False, stimulus=False)
...
def extract_area_dict(d, structure, target_area, source_area):
    ...
    for pop in structure[target_area]:
        ...
        for pop2 in structure[source_area]:
            area_dict[pop][pop2] = d[target_area][pop[:-1]][pop[-1]][source_area][pop2[:-1]][pop2[-1]]
    return area_dict
